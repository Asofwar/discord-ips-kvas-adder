...
2. Скачайте или клонируйте репозиторий на ваше устройство. Можно таким образом:
    ```bash
...
5. Следуйте инструкциям в терминале:
   ...
   - Нажмите Enter, чтобы оставить значение по умолчанию, или введите новый аргумент (_имя листа IPset_).
...
```bash
    # ./discord-ips-kvas-adder.sh good_list
...
- Возможные варианты:
  - `auto` — автоматический режим с использованием списка по умолчанию `unblock` (_в скрипте ipset-adder.sh пока ещё не реализовано определение версии КВАСа_)
  - `list <имя>` — автоматический режим с использованием указанного списка IPset (_рекомендуемый способ для бета-версии КВАСа_)

При запуске `discord-ips-kvas-adder.sh` вы можете указать этот аргумент или изменить его в интерактивном режиме
